...
### 10.8 Skills as a first-class entity

Until now `Job.skills` and `SeekerProfile.skills` were comma-separated `varchar(400)` columns. Three things followed from that, and all three were wrong:

| Symptom | Cause |
|---|---|
| Searching **java** returned every **JavaScript** job | `JobSpecifications.keyword` did `like '%java%'` over the skills text, and a substring test cannot tell one from the other. |
| A seeker's **Node.js** did not match a job's **Node JS** | `RecommendationScorer` rule 1 compared lower-cased *spellings*. Section 7.8's own case E7 says those are the same skill — and rule 3 duly matched them in a job's description text, so the scorer disagreed with itself. |
| "Java (24)" was impossible | Counting jobs per skill cannot be done against text without also counting the near-misses. |

Skills are now their own entity, shared by every job and every profile that lists them.

**The model**

```
skills(id, slug unique, label)
job_skills(job_id, skill_id, display_order)
seeker_profile_skills(seeker_profile_id, skill_id, display_order)
```

`slug` is the identity and the only thing ever compared; `label` is what gets rendered. `slug` is defined as **`TextMatcher.normalise(label)`** — lower-cased, every character other than a letter, digit, `+` or `#` replaced by a space, runs collapsed. That is deliberately the *same function* `containsPhrase` already used to look for a skill inside a title or description, so "the identity of a skill" and "the token searched for in free text" cannot drift apart. `Node.js`, `node js` and `NODE-JS` are one skill; `c`, `c++` and `c#` stay three.

`display_order` is there because the order a person typed their skills in is part of what they wrote — the chips on a job, and the list in "Matches your skills: Java, Spring Boot", both read back in that order. It is mapped as an `@OrderColumn` list, always replaced wholesale, never edited in the middle.

**Free text stays free text.** `SkillService.resolve` creates a row for any canonical key it has not seen, so an employer hiring for something nobody has posted before still types it and it works first time. This is **not** an admin-curated vocabulary. What changed is that the second person to type the same thing, in any spelling, lands on the same row. The first spelling to create a row becomes its label, so a later job that types `JAVA` renders `Java`.

**Normalisation rules, stated once**

* split on commas; an entry blank after trimming is dropped;
* `label` = the entry trimmed, inner whitespace collapsed to one space;
* `slug` = `label` canonicalised as above;
* duplicates within one list are removed by slug, first spelling kept;
* case, punctuation and spacing are normalised away. **Near-synonyms are not.** `JS`/`JavaScript`, `Postgres`/`PostgreSQL` stay separate skills: folding them needs a curated dictionary, and one wrong entry silently rewrites what a candidate said about themselves. Punctuation variants are merged because the app already treated them as the same phrase; judgements about *meaning* are left to people.

**What changed elsewhere**

* `JobSpecifications.keyword` — the skills branch matches **whole tokens** against `slug` (four LIKE patterns: whole slug, first word, last word, middle word). So `java` no longer matches `JavaScript`, while `spring` still matches `Spring Boot`, which a search box has to do. Title, description and company name keep their `like '%text%'`: those are prose, and a substring search over prose is correct. Every pattern is still built in Java, never assembled by the database — see the note under 10.7 about `concat` and PostgreSQL.
* `JobSpecifications.hasSkill(slug)` + `JobSearchCriteria.skill` — the faceted filter. A subquery, not a join, so a paged result is not multiplied.
* `JobRepository` gains the `JobSkillFacets` fragment: the counts are built with the Criteria API from **the same `Specification` the search itself used**, and computed *before* the skill filter is applied, so picking "Java" does not hide every other chip.
* `RecommendationScorer` rule 1 compares `Skill.slug`. **No published score changes**: rule 1 was already exact equality, so it only gains matches that used to fall through to rule 2 or 3, and only for spelling variants of a skill the job really lists. Cases E1–E7 and the Priya worked example are unchanged.
* `Job.skills` is `@BatchSize(50)`: the recommender scores 200 candidates per request, and a plain lazy collection would be 200 extra queries where the old column was free.

**The cutover: the old columns are kept**

`jobs.skills` and `seeker_profiles.skills` are still there, and V4 does not touch them. Dropping them is tidier but one-way: the moment it runs, the previous release can no longer read anybody's skills and "roll back" stops being an option. For the largest structural change since V1, one release of insurance is worth a redundant column.

The usual objection to keeping a column is drift, so the mapping removes the possibility rather than promising care: **nothing in the application reads those columns** (neither entity exposes a getter), and the single method that changes skills — `assignSkills` — writes the relation and the column together. A column nothing reads cannot drift into a wrong answer; at worst it is stale, which is what a rollback artefact is allowed to be. `JobTest` asserts it is still being written, because no other test in the suite would notice if it stopped.

**Rollback** is: deploy the previous release, run nothing. Flyway's history keeps its V4 row, which the older application ignores; the three new tables sit unused; both columns are correct for every row that existed before the upgrade and for every row saved after it. Re-upgrading later finds V4 already applied and needs no second backfill. The old columns can be dropped in a later migration once this release has held.

**V4 migrates the data, not just the schema**

`V4__skills_entity.sql` parses both columns and backfills. The parts worth knowing:

* it is written **without a regular expression**. `REGEXP_REPLACE(s, '[^a-z0-9+#]+', ' ', 'g')` cannot be used: H2 replaces every match and *rejects* the `g` flag, while PostgreSQL replaces only the **first** match unless given it. There is no single call that is global on both, so the regex form would canonicalise correctly in tests and mangle every multi-word skill in production. Instead a recursive CTE walks the string one character at a time — a direct transliteration of `TextMatcher.normalise`'s own loop.
* **nothing non-blank is ever dropped.** The 30-skill cap is a form-entry rule and is not applied (deleting skill 31 from an existing profile is data loss to enforce a limit on input that is not being entered). An entry of pure punctuation is kept as its own inert skill rather than this migration deciding, with nobody to ask, that something a person typed was meaningless.
* that rule is what makes the **guard** at the end exact: "a row whose old column held something other than whitespace, and which now has no join rows" is unambiguously a bug. It is asserted by inserting `NULL` into a `NOT NULL` column when the count is non-zero — the only way plain SQL can fail on purpose on both databases. The migration rolls back and the deploy stops with the previous version still serving, per 10.7's "a stopped deploy is the correct outcome".
...
* The facet's tie-break is by label, so two databases with different collations disagree about whether `CSS` sorts before `Content Writing`. Harmless, and no test asserts on it.
* The character walk keeps non-ASCII characters in a slug, where `TextMatcher` keeps letters and digits but turns exotic punctuation into a space. For any real skill the two agree; where they could not, the migration errs towards keeping the data.
---
...
| `SkillParserTest` | Unit | Trimming, blanks, 30-skill limit, first spelling kept; duplicates by **canonical key**, so `Node.js`/`node js`/`NODE-JS` are one skill while `c`/`c++`/`c#` stay three; a punctuation-only entry has no identity and is dropped from new input (10.8) | E-F1, S-F4, S-D5 |
...
| `JobSpecificationsTest` | Integration | `live` excludes pending, rejected, closed, expired, hidden; `notAppliedBy`; `keyword` escapes `%` and `_`; `#keywordMatchesSkillsAsWholeTokensNotSubstrings` (searching "java" no longer returns a JavaScript job, while "spring" still finds "Spring Boot"); `#hasSkillMatchesOnlyTheExactSkill` | S-F1, S-D5, 10.8 |
| `SkillFacetTest` | Integration | The faceted skill filter of 10.8: counts are per job and respect every other filter in the same search, the selected facet does not hide the others, an unknown slug is ignored like any other unparsable filter, and the chips reach the page with their counts | S-F1, 10.8 |
| `SkillBackfillMigrationTest` | Integration (no Spring context) | The half of V4 the rest of the suite cannot see: migrates to **V1 only**, inserts comma-separated rows the way the pre-10.8 application would have, then runs the remaining migrations and checks what came out — one row per canonical key, the most-used spelling as the label, order preserved and renumbered without gaps, punctuation-only entries kept rather than dropped, and the guard that stops the deploy if anything did not move | 10.8, 10.7 |
